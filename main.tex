...
\begin{fixedwidetable}{tab:related-work}{관련 연구와 본 논문의 위치.}
\scriptsize
...
\begin{fixedwidetable}{tab:dataset-config}{Distance별 데이터 구성. 원본 dataset은 family별 \(2048\) shots이고, 학습용
...
\begin{fixedwidetable}{tab:noise-family}{실험에 사용한 noise family.}
\footnotesize
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}llll}
\toprule
...
}{fig:pipeline}{제안하는 neural pre-decoder와 PyMatching 결합 구조. 신경망은 local
...
}{fig:architecture}{Neural pre-decoder의 구조. \texttt{SyndromeEditPreDecoder}가 shared 3D
...
\begin{fixedwidetable}{tab:main-results}{Held-out \texttt{stage\_c\_corr}에서의 main accuracy.}
\scriptsize
...
}{fig:main-accuracy}{거리별 held-out accuracy. d3/d5에서는 선택 모드의 pre-decoder가 raw
...
\begin{fixedwidetable}{tab:d5-fallback}{d5 seed별 선택 결과와 원본 유지 여부.}
\scriptsize
...
}{fig:d7-oracle}{d7의 oracle 기준 회복 가능성과 false-positive 후보 문제. 후보 공간에는
...
\begin{fixedtable}{tab:d7-mismatch}{d7 validation class와 held-out outcome의 불일치.}
\footnotesize
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}lccc}
\toprule
...
}{fig:d7-scatter}{d7 validation delta와 held-out candidate delta의 seed-level scatter.
...
}{fig:oracle-recovery}{Seed-level oracle recovery. d3/d5는 oracle 기준 회복 가능성의 일부를 실제
