...
Table~\ref{tab:related-work}은 관련 연구와 본 논문의 위치를 정리한다.
...
\begin{table*}[!tbp]
\caption{\thesiscaptionfont 관련 연구와 본 논문의 위치.}
...
한다. Table~\ref{tab:noise-family}에 세 noise family를 정리했다.
...
Table~\ref{tab:dataset-config}에 distance별 데이터 구성을 정리했다.

\begin{table*}[!tbp]
\caption{\thesiscaptionfont Distance별 데이터 구성. 원본 dataset은 family별 \(2048\) shots이고, 학습용
...
\begin{table*}[!tbp]
\caption{\thesiscaptionfont 실험에 사용한 noise family.}
...
\caption{\thesiscaptionfont 제안하는 neural pre-decoder와 PyMatching 결합 구조. 신경망은 local
...
\caption{\thesiscaptionfont Neural pre-decoder의 구조. \texttt{SyndromeEditPreDecoder}가 shared 3D
...
\begin{table*}[!tbp]
\caption{\thesiscaptionfont Held-out \texttt{stage\_c\_corr}에서의 main accuracy.}
...
\caption{\thesiscaptionfont 거리별 held-out accuracy. d3/d5에서는 선택 모드의 pre-decoder가 raw
...
d3는 seed \(0..7\) 전부에서 양의 정확도 변화를 보였다. Table~\ref{tab:d3d5-robustness}의 seed classes는
...
\(0.0039\)인 반면, d5는 \(0.25\)이다~(Table~\ref{tab:paired-stats}). 따라서 d3는 검사한 seed 전부에서 양의
...
\begin{table}[!tbp]
\caption{\thesiscaptionfont d5 seed별 선택 결과와 원본 유지 여부.}
...
활용하지도 못한다. Figure~\ref{fig:oracle-recovery}는 거리별 oracle gap의 회복
비율을 보여준다. 아래 결과에서는 candidate, selected, oracle branch를 구분해
표기한다.
...
\caption{\thesiscaptionfont d7의 oracle 기준 회복 가능성과 false-positive 후보 문제. 후보 공간에는
...
가리킨다. Table~\ref{tab:d7-mismatch}는 이 불일치를 정리한다.
...
\(13/22=59.09\%\)이다. Figure~\ref{fig:d7-scatter}는 seed별 validation/held-out
delta 산점도로 이 관계를 보여준다.

\begin{table}[!tbp]
\caption{\thesiscaptionfont d7 validation class와 held-out outcome의 불일치.}
...
\caption{\thesiscaptionfont d7 validation delta와 held-out candidate delta의 seed-level scatter.
...
\caption{\thesiscaptionfont Seed-level oracle recovery. d3/d5는 oracle 기준 회복 가능성의 일부를 실제
...
통과했으며, 실패 항목은 \(0\)개였다. 관련 summary와 재현성 메모는 저장소의
재현성 패키지에 보존했다. Hyperparameter sensitivity 기록은 본문 표에는
포함하지 않고 별도 보충 메모로 보존했다.
