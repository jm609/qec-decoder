...
\newcommand{\thesiscaptionfont}{\footnotesize}
...
the final logical decision. An important distinction is which decision the
neural model makes directly. The AlphaQubit family and CNN decoders predict the
final logical decision or correction directly from the
syndrome~\cite{alphaqubit,alphaqubit2,convnnsurface}, whereas this work leaves
the final matching problem to PyMatching and only decides whether to edit its
input locally. NVIDIA Ising-Decoding is the closest structural reference, since
it also combines a pre-decoder with a downstream decoder~\cite{nvidiaising}, but
here the candidate-generation and candidate-selection failure paths are examined
more closely at small code distances. The Google Willow results provide
hardware-scale QEC context~\cite{willowthreshold}, but they are not directly
compared against the numbers reported here. The numerical claims below are limited to the
Stage A/B/C noise families and the held-out \texttt{stage\_c\_corr} evaluation.

\begin{table*}[!tbp]
\caption{\thesiscaptionfont Position of this work relative to related decoding studies.}
...
evaluation. Table~\ref{tab:noise-family} summarizes the three noise families.
...
used to train the trunk and selector. Table~\ref{tab:dataset-config} summarizes
the dataset configuration by code distance.

\begin{table*}[!tbp]
\caption{\thesiscaptionfont Dataset configuration by code distance. Each original dataset contains
...
\begin{table*}[!tbp]
\caption{\thesiscaptionfont Noise families used in the experiments.}
...
\caption{\thesiscaptionfont Pipeline of the proposed neural pre-decoder and PyMatching. The neural
...
The trunk is trained with the Adam optimizer. The default configuration uses
batch size \(64\), learning rate \(10^{-3}\), and at most \(8\) epochs. The loss
has three parts. First, a masked binary cross-entropy is applied at detector
positions that have a known local-edit target. Second, a binary cross-entropy is
applied to the shot-level \texttt{needs\_edit\_logits}. Third, for shots that
need an edit, a ranking loss with margin \(1.0\) encourages the target edit to
score above the identity candidate. The reported numbers use the checkpoint from
the epoch with the best validation-family metric.
...
observed local-edit patterns are grouped into at most \(16\) motif classes, and
at inference the motif placements with the largest summed per-detector edit
probability are kept as the top \(32\) candidates. The
...
benefit/harm ranking loss of weight \(1.0\) and margin \(0.5\). Candidates that
damage an already-correct shot are given harm weight \(2.0\), while nonzero
candidates that leave a raw-incorrect shot still incorrect are given miss
weight \(0.25\), so harmful candidates are weighted more strongly than missed
beneficial candidates. This makes the selector a decoder-aware ranking module. Figure~\ref{fig:architecture}
summarizes the model structure.

\begin{figure*}[!tbp]
\centering
\predecoderfigure{fig2_model_architecture}{\textwidth}
\caption{\thesiscaptionfont Architecture of the neural pre-decoder. The
...
no-edit. Concretely, a validation delta of at least \(+0.02\) over raw no-edit
is treated as sufficient evidence for adoption; a weak positive case is adopted
only when the validation delta is at least \(+0.005\), the selector margin is at
least \(0.5\), and the harmed count is at most \(2\); otherwise no edit is
applied. The fact that only seeds 2 and 3 adopt local edits at d5, while the
remaining seeds keep the raw syndrome, follows directly from this rule. This
safety rule reflects the fact that local edits can help some seeds while harming
others.
...
\begin{table*}[!tbp]
\caption{\thesiscaptionfont Main accuracy on held-out \texttt{stage\_c\_corr}.}
...
\caption{\thesiscaptionfont Held-out accuracy by code distance. The selected predecoder improves
...
Table~\ref{tab:d3d5-robustness} summarizes the d3/d5 robustness results. Here,
seed classes denote positive/neutral/negative selected-delta seed counts.
...
\begin{table}[!tbp]
\caption{\thesiscaptionfont D3/D5 seed \(0..7\) robustness summary. Seed classes are
...
test, the one-sided p-value is \(0.0039\) for d3 and \(0.25\) for d5
(Table~\ref{tab:paired-stats}). Thus, the
...
\begin{table}[!tbp]
\caption{\thesiscaptionfont D3/D5 paired seed-level statistics.}
...
\begin{table}[!tbp]
\caption{\thesiscaptionfont D5 seed-level selected-mode fallback behavior.}
\label{tab:d5-fallback}
\centering
\scriptsize
\setlength{\tabcolsep}{2.4pt}
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}clrrl}
\toprule
seed & mode & sel. \(\Delta\) & cand. \(\Delta\) & note \\
\midrule
...
it also misses most recoverable edits found by the oracle analysis
(Figure~\ref{fig:oracle-recovery}). The results
...
\caption{\thesiscaptionfont D7 oracle recoverability and false-positive candidate problem. Useful edits remain
...
The validation-to-held-out mismatch shows the same issue at the seed level
(Table~\ref{tab:d7-mismatch}). Among 22
...
validation-positive seeds is \(13/22=59.09\%\). Figure~\ref{fig:d7-scatter}
plots this seed-level relationship between validation and held-out candidate
delta.

\begin{table}[!tbp]
\caption{\thesiscaptionfont Mismatch between d7 validation class and held-out outcome.}
...
\begin{figure}[!tbp]
\centering
\predecoderfigure{fig5_d7_validation_heldout_scatter}{\columnwidth}
\caption{\thesiscaptionfont Seed-level scatter of d7 validation delta versus held-out candidate
...
\begin{figure}[!tbp]
\centering
\predecoderfigure{fig6_oracle_recovery_distribution}{\columnwidth}
\caption{\thesiscaptionfont Seed-level oracle recovery. D3 and d5 convert part of the recoverable
