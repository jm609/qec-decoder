...
\makeatletter
\newcommand{\fixedcaption}[3]{%
    \par\vspace{0.45em}%
    \refstepcounter{#1}\label{#2}%
    \begin{minipage}{0.92\linewidth}%
    \small\textbf{\csname fnum@#1\endcsname.} #3%
    \end{minipage}\par
}
\newenvironment{fixedtable}[2]{%
    \begin{center}%
    \fixedcaption{table}{#1}{#2}%
    \vspace{0.35em}%
}{%
    \end{center}%
}
\newenvironment{fixedwidetable}[2]{%
    \begin{widetext}%
    \begin{center}%
    \fixedcaption{table}{#1}{#2}%
    \vspace{0.35em}%
...
\newcommand{\fixedwidefigure}[3]{%
    \begin{widetext}%
    \begin{center}%
    #1%
    \fixedcaption{figure}{#2}{#3}%
    \end{center}%
    \end{widetext}%
}
\makeatother
...
\begin{fixedwidetable}{tab:related-work}{Position of this work relative to related decoding studies.}
\scriptsize
...
\begin{fixedwidetable}{tab:dataset-config}{Dataset configuration by code distance. Each original dataset contains
...
\begin{fixedwidetable}{tab:noise-family}{Noise families used in the experiments.}
\footnotesize
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}llll}
\toprule
...
}{fig:pipeline}{Pipeline of the proposed neural pre-decoder and PyMatching. The neural
...
}{fig:architecture}{Architecture of the neural pre-decoder. The
...
\begin{fixedwidetable}{tab:main-results}{Main accuracy on held-out \texttt{stage\_c\_corr}.}
\scriptsize
...
}{fig:main-accuracy}{Held-out accuracy by code distance. The selected predecoder improves
...
\begin{fixedtable}{tab:d3d5-robustness}{D3/D5 seed \(0..7\) robustness summary. Seed classes are
...
\begin{fixedtable}{tab:paired-stats}{D3/D5 paired seed-level statistics.}
\footnotesize
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}lccc}
\toprule
...
\begin{fixedwidetable}{tab:d5-fallback}{D5 seed-level selected-mode fallback behavior.}
\scriptsize
...
}{fig:d7-oracle}{D7 oracle recoverability and false-positive candidate problem. Useful edits remain
...
\begin{fixedtable}{tab:d7-mismatch}{Mismatch between d7 validation class and held-out outcome.}
\footnotesize
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}lccc}
\toprule
...
}{fig:d7-scatter}{Seed-level scatter of d7 validation delta versus held-out candidate
...
}{fig:oracle-recovery}{Seed-level oracle recovery. D3 and d5 convert part of the recoverable
