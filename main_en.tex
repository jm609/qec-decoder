...
The result tables are based on the archived pre-decoder summary documents in
the repository, including the final result tables, paired statistics,
oracle-recovery distribution, d7 harmful-edit taxonomy, and reproducibility
package. The final consistency check is recorded under
\texttt{artifacts/eval/nn/} with \(37/37\) checks and \(0\) failures.
Hyperparameter-sensitivity notes are archived separately in
\texttt{PREDECODER\_HYPERPARAMETER\_SENSITIVITY.md}.
